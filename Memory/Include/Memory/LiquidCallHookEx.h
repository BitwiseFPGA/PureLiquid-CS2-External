...
public:
        ...
        uint8_t              overwriteSize = 0)
    {
        ...
        // ── locate module ────────────────────────────────────────────────
        auto pMod = m_pProc->GetRemoteModule(dllName.c_str());
        ...
        m_pDataRemote = m_pProc->Alloc(sizeof(HOOK_DATA));
        if (!m_pDataRemote) {
            printf("[!] %s: failed to alloc hook data\n", m_szName.c_str());
            return false;
        }

        m_pProc->Write<HOOK_DATA>(reinterpret_cast<uintptr_t>(m_pDataRemote), initData);
        ...
        m_pShellcodeRemote = m_pProc->Alloc(
        ...
        // ── flush shellcode ───────────────────────────────────────────────
        if (!FlushInstructionCache(m_pProc->m_hProc,
    ...
    bool Unhook()
    {
        ...
        DWORD oldProtect;
        if (!VirtualProtectEx(m_pProc->m_hProc,
            ...
            if (!m_pProc->WriteArray(m_callSiteAddr, origVec)) {
                printf("[!] %s: failed to restore original bytes\n",
                    m_szName.c_str());
                success = false;
            }
            VirtualProtectEx(m_pProc->m_hProc,
                reinterpret_cast<void*>(m_callSiteAddr),
                m_originalByteCount, oldProtect, &oldProtect);
        }

        FlushInstructionCache(m_pProc->m_hProc,
            reinterpret_cast<void*>(m_callSiteAddr), m_originalByteCount);

        // Free remote allocs
        if (m_pFuncPtrStorage) {
            VirtualFreeEx(m_pProc->m_hProc, m_pFuncPtrStorage, 0, MEM_RELEASE);
    ...
    HOOK_DATA ReadData()
    {
        HOOK_DATA out{};
        if (m_pDataRemote)
            m_pProc->Read(reinterpret_cast<uintptr_t>(m_pDataRemote),
...
private:
        ...
        const std::vector<RipSlot>& ripSlots)
    {
        ...
        for (size_t i = 0; i + 7 <= shellcodeSize; ++i) {
            ...
            if (!m_pProc->Write<int32_t>(remoteInstrAddr + 3, newOffset)) {
    ...
    bool InstallCallSitePatch()
    {
        ...
        for (uintptr_t addr = searchStart; addr < searchEnd; addr += granularity) {
            m_pFuncPtrStorage = VirtualAllocEx(m_pProc->m_hProc,
        ...
        bool ok = m_pProc->WriteArray(m_callSiteAddr, patch);
    ...
    bool TryRestore()
    {
        uint32_t currentPid = m_pProc->GetProcId();
        ...
        uint8_t probe[2]{};
        m_pProc->Read(entry->callSiteAddr, probe, 2);
        ...
        for (const auto& saved : entry->ripSlots) {
            ...
            uint64_t storedValue = m_pProc->ReadDirect<uint64_t>(saved.remoteAddr);
            ...
            case RipSlotTarget::Custom:
                if (storedValue != saved.customAddr) {
                    ...
                    m_pProc->Write<uint64_t>(saved.remoteAddr, saved.customAddr);
                }
                break;
            }
    ...
    void SaveConfig()
    {
        ...
        e.ripSlots.clear();
        for (const auto& rs : m_RemoteSlots) {
            HookConfig::RipSlotEntry saved;
            saved.remoteAddr = reinterpret_cast<uintptr_t>(rs.remoteAddr);
            saved.target = static_cast<HookConfig::RipSlotTarget>(rs.target);
