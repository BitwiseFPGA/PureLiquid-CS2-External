...
public:
        ...
        std::vector<RipSlot> ripSlots)
    {
        ...
        m_pOrigStorage = m_pProc->Alloc(8);
        if (!m_pOrigStorage) {
    ...
    bool Unhook()
    {
        ...
        uint64_t originalFunc = m_pProc->ReadDirect<uint64_t>(
        ...
        m_pProc->Write<uint64_t>(m_pTargetFunction, originalFunc);
...
private:
    ...
    bool TryRestore()
    {
        ...
        uint64_t currentVtableEntry =
            m_pProc->ReadDirect<uint64_t>(entry->targetFunction);
        ...
        uint64_t origFuncValue = m_pProc->ReadDirect<uint64_t>(entry->origStorage);
    ...
    bool InstallVTableHook()
    {
        ...
        bool ok = m_pProc->Write<uint64_t>(
            m_pTargetFunction,
