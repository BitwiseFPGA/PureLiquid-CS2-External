// generated - do not edit!
// generated by @xsip cs2 schema generator

#pragma once

#ifndef CUSTOM_GLOBAL_TYPES
	#include <SDK/GlobalTypes.hpp>
#else
	#include <Custom/GlobalTypes.hpp>
#endif


#include <SDK/server/CBtNode.hpp>
#include <SDK/server/CountdownTimer.hpp>





using namespace GlobalTypes;
namespace CS2 {
	namespace server {
		class CBtActionAim : public CS2::server::CBtNode {
		public:
			PROPERTY(m_szSensorInputKey,GlobalTypes::CUtlString* , 0x68);
			PROPERTY(m_szAimReadyKey,GlobalTypes::CUtlString* , 0x80);
			PROPERTY(m_flZoomCooldownTimestamp,float32 , 0x88);
			PROPERTY(m_bDoneAiming,bool , 0x8c);
			PROPERTY(m_flLerpStartTime,float32 , 0x90);
			PROPERTY(m_flNextLookTargetLerpTime,float32 , 0x94);
			PROPERTY(m_flPenaltyReductionRatio,float32 , 0x98);
			PROPERTY(m_NextLookTarget,GlobalTypes::QAngle , 0x9c);
			PROPERTY(m_AimTimer,server::CountdownTimer , 0xa8);
			PROPERTY(m_SniperHoldTimer,server::CountdownTimer , 0xc0);
			PROPERTY(m_FocusIntervalTimer,server::CountdownTimer , 0xd8);
			PROPERTY(m_bAcquired,bool , 0xf0);
			S2_PAD(0xA0);
		};
		//static_assert(sizeof(CS2::server::CBtActionAim) == 0xF8, "CBtActionAim size should be 0xF8");
	}
}
