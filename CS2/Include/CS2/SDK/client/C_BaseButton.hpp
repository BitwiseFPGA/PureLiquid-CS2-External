// generated - do not edit!
// generated by @xsip cs2 schema generator

#pragma once

#ifndef CUSTOM_GLOBAL_TYPES
	#include <SDK/GlobalTypes.hpp>
#else
	#include <Custom/GlobalTypes.hpp>
#endif


#include <SDK/client/C_BaseToggle.hpp>



namespace CS2 {
	namespace client {
		class C_BaseModelEntity;
	}
}


using namespace GlobalTypes;
namespace CS2 {
	namespace client {
		class C_BaseButton : public CS2::client::C_BaseToggle {
		public:
			PROPERTY(m_glowEntity,GlobalTypes::CHandle<client::C_BaseModelEntity>, 0xeb0);
			PROPERTY(m_usable,bool , 0xeb4);
			PROPERTY(m_szDisplayText,GlobalTypes::CUtlSymbolLarge* , 0xeb8);
			S2_PAD(0x10);
		};
		//static_assert(sizeof(CS2::client::C_BaseButton) == 0xEC0, "C_BaseButton size should be 0xEC0");
	}
}
