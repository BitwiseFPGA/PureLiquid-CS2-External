// generated - do not edit!
// generated by @xsip cs2 schema generator

#pragma once

#ifndef CUSTOM_GLOBAL_TYPES
	#include <SDK/GlobalTypes.hpp>
#else
	#include <Custom/GlobalTypes.hpp>
#endif





namespace CS2 {
	namespace modellib {
		class ModelMeshBufferData_t;
	}
}


using namespace GlobalTypes;
namespace CS2 {
	namespace modellib {
		class ModelEmbeddedMesh_t  {
		public:
			PROPERTY(m_Name,GlobalTypes::CUtlString* , 0x0);
			PROPERTY(m_nMeshIndex,int32_t , 0x10);
			PROPERTY(m_nDataBlock,int32_t , 0x14);
			PROPERTY(m_nMorphBlock,int32_t , 0x18);
			PROPERTY(m_vertexBuffers,GlobalTypes::CUtlVector<modellib::ModelMeshBufferData_t>, 0x20);
			PROPERTY(m_indexBuffers,GlobalTypes::CUtlVector<modellib::ModelMeshBufferData_t>, 0x38);
			PROPERTY(m_toolsBuffers,GlobalTypes::CUtlVector<modellib::ModelMeshBufferData_t>, 0x50);
			PROPERTY(m_nVBIBBlock,int32_t , 0x68);
			PROPERTY(m_nToolsVBBlock,int32_t , 0x6c);
			S2_PAD(0x70);
		};
		//static_assert(sizeof(CS2::modellib::ModelEmbeddedMesh_t) == 0x70, "ModelEmbeddedMesh_t size should be 0x70");
	}
}
