// generated - do not edit!
// generated by @xsip cs2 schema generator

#pragma once

#ifndef CUSTOM_GLOBAL_TYPES
	#include <SDK/GlobalTypes.hpp>
#else
	#include <Custom/GlobalTypes.hpp>
#endif


#include <SDK/animationsystem/CSeqSeqDescFlag.hpp>
#include <SDK/animationsystem/CSeqTransition.hpp>



namespace CS2 {
	namespace animationsystem {
		class CSeqCmdLayer;
		class CAnimEventDefinition;
		class CAnimActivity;
		class CSeqPoseSetting;
	}
}


using namespace GlobalTypes;
namespace CS2 {
	namespace animationsystem {
		class CSeqCmdSeqDesc  {
		public:
			PROPERTY(m_sName,GlobalTypes::CBufferString , 0x0);
			PROPERTY(m_flags,animationsystem::CSeqSeqDescFlag , 0x10);
			PROPERTY(m_transition,animationsystem::CSeqTransition , 0x1c);
			PROPERTY(m_nFrameRangeSequence,int16_t , 0x24);
			PROPERTY(m_nFrameCount,int16_t , 0x26);
			PROPERTY(m_flFPS,float32 , 0x28);
			PROPERTY(m_nSubCycles,int16_t , 0x2c);
			PROPERTY(m_numLocalResults,int16_t , 0x2e);
			PROPERTY(m_cmdLayerArray,GlobalTypes::CUtlVector<animationsystem::CSeqCmdLayer>, 0x30);
			PROPERTY(m_eventArray,GlobalTypes::CUtlVector<animationsystem::CAnimEventDefinition>, 0x48);
			PROPERTY(m_activityArray,GlobalTypes::CUtlVector<animationsystem::CAnimActivity>, 0x60);
			PROPERTY(m_poseSettingArray,GlobalTypes::CUtlVector<animationsystem::CSeqPoseSetting>, 0x78);
			S2_PAD(0x90);
		};
		//static_assert(sizeof(CS2::animationsystem::CSeqCmdSeqDesc) == 0x90, "CSeqCmdSeqDesc size should be 0x90");
	}
}
