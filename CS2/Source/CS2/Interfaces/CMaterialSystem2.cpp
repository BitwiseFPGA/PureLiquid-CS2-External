...
#pragma check_stack(off)  
	__declspec(safebuffers)
		...
		::Source2::CKeyValues3* kv = (::Source2::CKeyValues3*)(reinterpret_cast<uint8_t*>(ctx->kv));

		for (int i = 0; i < sizeof(::Source2::CKeyValues3); i++) {
			reinterpret_cast<uint8_t*>(kv)[i] = 0;
		}

		::Source2::KV3ID_t kvId;

		kvId.szName = ctx->szKvIdName;
		kvId.unk0 = 0x469806E97412167CULL;
		kvId.unk1 = 0xE73790B53EE6F2AFULL;

		typedef bool(__fastcall* LoadKV3Fn)(::Source2::CKeyValues3*, void*, const char*, void*, void*);
		...
		using CreateMaterialFn = void (*)(void*, CMaterial2**, const char*, ::Source2::CKeyValues3*, int, int);
		...
		if (mat) {
			ctx->pCreatedMaterial = reinterpret_cast<::Source2::CStrongHandle<CMaterial2>*>(mat);
			ctx->bHadErrors = 0;
			ctx->bFinished = 1;
			return 1;
		}
...
#pragma code_seg()
	...
	::Source2::CStrongHandle<CMaterial2>* CMaterialSystem2::CreateMaterial(std::string materialKv3Str, std::string szMatName) {
		...
		ctx.kv = reinterpret_cast<::Source2::CKeyValues3*>(reinterpret_cast<uintptr_t>(pKv3Remote) + 0x100);
